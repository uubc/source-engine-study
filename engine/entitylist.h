...
extern IStaticPropMgrServer* pServerStaticPropMgr;
...
extern IEffects* g_pServerEffects;
#ifdef POSIX
...
public:
	...
	:m_pServerEntityList(pServerEntityList), m_RefEHandle(iForceEdictIndex, iSerialNum), m_Network(this)
	{
		...
		m_flElasticity = 1.0f;
		SetFriction(1.0f);
		m_nLastThinkTick = g_ServerGlobalVariables.tickcount;
...
{
public:
	CVehicleSoundsParser(IPooledStringAllocer* pAllocer);
...
private:
	...
	IPooledStringAllocer* m_pAllocer;
};
...
public:
	...
	virtual IEffects* GetEffects() {
		return g_pServerEffects;
	}
...
// when people call GetAbsAnything. 
	...
	void UpdateTestMoveTypeStepSimulation();
...
//-----------------------------------------------------------------------------
	...
	void PhysFrictionSound(IHandleEntity* pEntity, IPhysicsObject* pObject, const char* pSoundName, HSOUNDSCRIPTHANDLE& handle, float flVolume)
	{
		...
		if (flVolume > (1.0f / 128.0f))
		{
			...
			pFriction->flLastEffectTime = g_ServerGlobalVariables.curtime;
		}
	}
...
//-----------------------------------------------------------------------------
	...
	void OutputVPhysicsBudgetInfo() {
		...
		if (activeCount)
		{
			...
			float full = g_pVEngineServer->Time();
			m_pPhysenv->Simulate(g_ServerGlobalVariables.interval_per_tick);
			full = g_pVEngineServer->Time() - full;
			float lastTime = full;
			...
			for (i = 0; i < ents.Count(); i++)
			{
				...
				float start = g_pVEngineServer->Time();
				m_pPhysenv->Simulate(g_ServerGlobalVariables.interval_per_tick);
				float end = g_pVEngineServer->Time();
	...
	IServerEntity* FindClientInPVS(const Vector& vecBoxMins, const Vector& vecBoxMaxs)
	{
		...
		if (!g_pVEngineServer->CheckBoxInPVS(vecBoxMins, vecBoxMaxs, m_checkPVS, sizeof(m_checkPVS)))
		{
			return NULL;
		}
	...
	IServerEntity* FindClientInPVSGuts(IServerEntity* pEdict, unsigned char* pvs, unsigned pvssize)
	{
		...
		if (pe)
		{
			...
			if (!g_pVEngineServer->CheckOriginInPVS(view, pvs, pvssize))
			{
				return NULL;
			}
		}
...
//-----------------------------------------------------------------------------
	IServerEntity* EntitiesInPVS(IServerEntity* pPVSEntity, IServerEntity* pStartingEntity)
	{
		...
		if (!pStartingEntity)
		{
			...
			int clusterIndex = g_pVEngineServer->GetClusterForOrigin(org);
			Assert(clusterIndex >= 0);
			g_pVEngineServer->GetPVSForCluster(clusterIndex, sizeof(pvs), pvs);
		}
		...
		for (IServerEntity* pEntity = NextEnt(pStartingEntity); pEntity; pEntity = NextEnt(pEntity))
		{
			...
			if (!g_pVEngineServer->CheckBoxInPVS(vecSurroundMins, vecSurroundMaxs, pvs, sizeof(pvs)))
				continue;
...
protected:
	...
	int GetNewCheckClient(int check)
	{
		...
		if (check < 1)
			check = 1;
		if (check > g_ServerGlobalVariables.maxClients)
			check = g_ServerGlobalVariables.maxClients;
		...
		for (; ; i++)
		{
			if (i > g_ServerGlobalVariables.maxClients)
			{
				i = 1;
			}
		...
		if (ent)
		{
			...
			if (clusterIndex != m_checkCluster)
			{
				...
				g_pVEngineServer->GetPVSForCluster(clusterIndex, sizeof(m_checkPVS), m_checkPVS);
			}
		}
...
//-----------------------------------------------------------------------------
	IServerEntity* GetCurrentCheckClient()
	{
		...
		float delta = g_ServerGlobalVariables.curtime - m_lastchecktime;
		if (delta >= 0.1 || delta < 0)
		{
			m_lastcheck = GetNewCheckClient(m_lastcheck);
			m_lastchecktime = g_ServerGlobalVariables.curtime;
		}
...
// if it contains any solid space?  or would that eliminate some entities we want to keep?
	int EntityInSolid(IServerEntity* ent)
	{
		...
		return (g_pEngineTraceServer->GetPointContents(point) & MASK_SOLID);
	}
...
bool CGlobalEntityList<T>::Init()
{
	...
	if ((m_physics = (IPhysics*)g_AppSystemFactory(VPHYSICS_INTERFACE_VERSION, NULL)) == NULL ||
		(m_pPhyscollision = (IPhysicsCollision*)g_AppSystemFactory(VPHYSICS_COLLISION_INTERFACE_VERSION, NULL)) == NULL ||
		(m_pPhysprops = (IPhysicsSurfaceProps*)g_AppSystemFactory(VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL)) == NULL
		)
		return false;

	PhysParseSurfaceData(m_pPhysprops, g_pFileSystem);
...
//#endif
	...
	//factorylist_t factories;
	//FactoryList_Retrieve(factories);
	m_pPhysenv->SetDebugOverlay(g_AppSystemFactory);
	...
	pServerStaticPropMgr->CreateVPhysicsRepresentations(m_pPhysenv, g_pSolidSetup, GetBaseEntity(0));
...
void CGlobalEntityList<T>::PhysFrame(float deltaTime)
{
	...
	if (bProfile)
	{
		simRealTime = g_pVEngineServer->Time();
	}
	...
	if (bProfile)
	{
		simRealTime = g_pVEngineServer->Time() - simRealTime;
...
void CGlobalEntityList<T>::FrameUpdatePostEntityThink()
{
	...
	float interval = (g_ServerGlobalVariables.frametime > 0.0f) ? TICK_INTERVAL : 0.0f;

	// update the physics simulation, not we don't use g_ServerGlobalVariables.frametime, since that can be 30 msec or 15 msec
...
bool CGlobalEntityList<T>::SaveInitEntities(CSaveRestoreData* pSaveData)
{
	...
	entitytable_t* pEntityTable = (entitytable_t*)g_pVEngineServer->SaveAllocMemory((sizeof(entitytable_t) * number_of_entities), sizeof(char));
	if (!pEntityTable)
		return false;
...
void CGlobalEntityList<T>::Save(ISave* pSave)
{
	...
	for (int i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		if (pEnt && !(pEnt->ObjectCaps() & FCAP_DONT_SAVE))
		{
			...
			bool bIsPlayer = ((nEntIndex >= 1) && (nEntIndex <= g_ServerGlobalVariables.maxClients)) ? true : false;
			if (bIsPlayer)
			{
...
void CGlobalEntityList<T>::ReadRestoreHeaders(IRestore* pRestore)
{
	...
	entitytable_t* pEntityTable = (entitytable_t*)g_pVEngineServer->SaveAllocMemory((sizeof(entitytable_t) * nEntities), sizeof(char));
	if (!pEntityTable)
	{
		return;
	}
...
void CGlobalEntityList<T>::Restore(IRestore* pRestore, bool createPlayers)
{
	...
	for (i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		if (pEntInfo->classname != NULL_STRING && pEntInfo->size && !(pEntInfo->flags & FENTTABLE_REMOVED))
		{
			...
			else if ((pEntInfo->edictindex > 0) && (pEntInfo->edictindex <= g_ServerGlobalVariables.maxClients))
			{
...
int CGlobalEntityList<T>::RestoreEntity(T* pEntity, IRestore* pRestore, entitytable_t* pEntInfo)
{
	...
	{
		int globalIndex = g_pVEngineServer->GlobalEntity_GetIndex(pEntity->GetEngineObject()->GetGlobalname());
		if (globalIndex >= 0)
		{
			// Already dead? delete
			if (g_pVEngineServer->GlobalEntity_GetState(globalIndex) == GLOBAL_DEAD)
				return -1;
		...
		else
		{
			...
			g_pVEngineServer->GlobalEntity_Add(pEntity->GetEngineObject()->GetGlobalname(), g_ServerGlobalVariables.mapname, GLOBAL_ON);
		}
	}
...
int CGlobalEntityList<T>::RestoreGlobalEntity(T* pEntity, IRestore* pRestore, entitytable_t* pEntInfo)
{
	...
	int globalIndex = g_pVEngineServer->GlobalEntity_GetIndex(globalName);

	// Don't overlay any instance of the global that isn't the latest
	// pSaveData->szCurrentMapName is the level this entity is coming from
	// pGlobal->levelName is the last level the global entity was active in.
	// If they aren't the same, then this global update is out of date.
	if (!datamap_t::FStrEq(pSaveData->levelInfo.szCurrentMapName, g_pVEngineServer->GlobalEntity_GetMap(globalIndex)))
	{
		return 0;
	}
	...
	if (pNewEntity)
	{
		...
		g_pVEngineServer->GlobalEntity_SetMap(globalIndex, g_ServerGlobalVariables.mapname);
	}
	else
	{
		// This entity will be freed automatically by the g_pVEngineServer->  If we don't do a restore on a matching entity (below)
...
void CGlobalEntityList<T>::CreateEntitiesInTransitionList(IRestore* pRestore, int levelMask)
{
	...
	for (i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		// spawn players
		pent = NULL;
		if ((pEntInfo->edictindex > 0) && (pEntInfo->edictindex <= g_ServerGlobalVariables.maxClients))
		{
...
int	CGlobalEntityList<T>::CreateEntityTransitionList(IRestore* pRestore, int a)
{
	...
	if (movedCount)
	{
		g_pVEngineServer->CallBlockHandlerRestore(&m_PhysSaveRestoreBlockHandler, base, pRestore, false);
		g_pVEngineServer->CallBlockHandlerRestore(serverGameDLL->GetAISaveRestoreBlockHandler(), base, pRestore, false);
	}
...
int CGlobalEntityList<T>::AddLandmarkToList(levellist_t* pLevelList, int listCount, const char* pMapName, const char* pLandmarkName, T* pentLandmark)
{
	...
	if (stricmp(pMapName, STRING(g_ServerGlobalVariables.mapname)) == 0)
		return 0;
...
bool CGlobalEntityList<T>::IsEntityInTransition(T* pEntity, const char* pLandmarkName)
{
	...
	int clusterIndex = g_pVEngineServer->GetClusterForOrigin(pLandmark->GetEngineObject()->GetAbsOrigin());
	g_pVEngineServer->GetPVSForCluster(clusterIndex, sizeof(pvs), pvs);
	Vector vecSurroundMins, vecSurroundMaxs;
	pEntity->GetEngineObject()->WorldSpaceSurroundingBounds(&vecSurroundMins, &vecSurroundMaxs);

	return g_pVEngineServer->CheckBoxInPVS(vecSurroundMins, vecSurroundMaxs, pvs, sizeof(pvs));
}
...
void CGlobalEntityList<T>::OnChangeLevel(const char* pNewMapName, const char* pNewLandmarkName) 
{
	...
	// This object will get removed in the call to g_pVEngineServer->ChangeLevel, copy the params into "safe" memory
...
inline IServerEntity* CGlobalEntityList<T>::CreateEntityByName(const char* className, int iForceEdictIndex, int iSerialNum) {
	...
	if (iForceEdictIndex == 0) {
	...
	else if (iForceEdictIndex >= 1 && iForceEdictIndex <= g_ServerGlobalVariables.maxClients) {
		...
		case ENGINEOBJECT_WORLD:
			Error("ENGINEOBJECT_WORLD handled by g_pVEngineServer!\n");
			break;
		case ENGINEOBJECT_PLAYER:
			Error("ENGINEOBJECT_PLAYER handled by g_pVEngineServer!\n");
			break;
...
IServerEntity* CGlobalEntityList<T>::GetPlayerByIndex(int playerIndex)
{
	...
	if (playerIndex > 0 && playerIndex <= g_ServerGlobalVariables.maxClients)
	{
...
{
	if (g_ServerGlobalVariables.maxClients > 1)
	{
...
template<class T>
void CGlobalEntityList<T>::UpdateTestMoveTypeStepSimulation()
{
...
bool CGlobalEntityList<T>::FindOrAddVehicleScript(const char* pScriptName, vehicleparams_t* pVehicle, vehiclesounds_t* pSounds)
{
	...
	{
		void* pFile = NULL;
		int length = g_pFileSystem->ReadFileEx(pScriptName, "GAME", &pFile, true, true);
		if (pFile)
		{
			...
			while (!pParse->Finished())
			{
				...
				{
					bLoadedSounds = true;
					CVehicleSoundsParser soundParser(this);
			...
			g_pFileSystem->FreeOptimalReadBuffer(pFile);
		}
	}
...
{
	m_impactSoundTime += g_ServerGlobalVariables.frametime;
