...
extern IEffects* g_pClientEffects;
#ifdef POSIX
...
extern CRopeManager* g_RopeManager;
...
class C_EngineObjectInternal : public IEngineObjectClient {
	friend class C_PortalCollideableEnumerator;
...
public:
	...
	C_EngineObjectInternal(IClientEntityList* pClientEntityList, int iForceEdictIndex, int iSerialNum)
		...
		m_iv_vecOrigin(g_ClientGlobalVariables.curtime, "IClientEntity::m_iv_vecOrigin", &m_vecOrigin, LATCH_SIMULATION_VAR),
		m_iv_angRotation(g_ClientGlobalVariables.curtime, "IClientEntity::m_iv_angRotation", &m_angRotation, LATCH_SIMULATION_VAR),
		m_iv_vecVelocity(g_ClientGlobalVariables.curtime, "IClientEntity::m_iv_vecVelocity", &m_vecVelocity, LATCH_SIMULATION_VAR),
		m_iv_flCycle(g_ClientGlobalVariables.curtime, "C_BaseAnimating::m_iv_flCycle", &m_flCycle, LATCH_ANIMATION_VAR),
		m_iv_flPoseParameter(g_ClientGlobalVariables.curtime, "C_BaseAnimating::m_iv_flPoseParameter", m_flPoseParameter, LATCH_ANIMATION_VAR),
		m_iv_flEncodedController(g_ClientGlobalVariables.curtime, "C_BaseAnimating::m_iv_flEncodedController", m_flEncodedController, LATCH_ANIMATION_VAR),
		m_iv_ragPos(g_ClientGlobalVariables.curtime, "C_ServerRagdoll::m_iv_ragPos", m_ragPos, LATCH_SIMULATION_VAR),
		m_iv_ragAngles(g_ClientGlobalVariables.curtime, "C_ServerRagdoll::m_iv_ragAngles", m_ragAngles, LATCH_SIMULATION_VAR)
	{
...
/////////////////
	...
	void Clear(void) {
		...
		ClearFlags();
		ClearEffects();
		m_nLastThinkTick = g_ClientGlobalVariables.tickcount;
...
{
	return modelinfoclient->GetModelType(m_pModel);
}
...
class C_CollisionEvent : public IPhysicsCollisionEvent, public IPhysicsCollisionSolver, public IPhysicsObjectEvent
{
public:
	C_CollisionEvent(void) = default;
...
private:
	...
	{
	public:
		CallbackContext(C_CollisionEvent* pOuter)
		{
			m_pOuter = pOuter;
...
class C_StaticCollisionPolyhedronCache
{
public:
	C_StaticCollisionPolyhedronCache(void);
	~C_StaticCollisionPolyhedronCache(void);
...
// this global keeps the last known server packet tick (to avoid calling engineClient->GetLastTimestamp() all the time)
...
//
// 1. It converts server entity indices into IClientNetworkables for the engineClient.
//
...
class CClientEntityList : public CBaseEntityList<T>, public IClientEntityList, public IEntityCallBack
{
	...
	friend class C_PortalTouchScope;
	friend class C_CollisionEvent;
...
public:
	...
	virtual IVModelInfo* GetModelInfo(){
		return modelinfoclient;
	}
	virtual IEffects* GetEffects() {
		return g_pClientEffects;
	}
	string_t AllocPooledString(const char* pStr) {
		return g_ClientDLL->AllocPooledString(pStr);
	}
...
public:
	...
	IClientEntity*			GetBaseEntity( int entnum ) const;
	...
	void PhysFrictionSound(IHandleEntity* pEntity, IPhysicsObject* pObject, const char* pSoundName, HSOUNDSCRIPTHANDLE& handle, float flVolume)
	{
		...
		if (flVolume > (1.0f / 128.0f))
		{
			...
			pFriction->flLastEffectTime = g_ClientGlobalVariables.curtime;
		}
	}
	...
	{
		return m_pPhysenv->GetSimulationTime() + g_ClientGlobalVariables.frametime * cl_phys_timescale.GetFloat();
	}
	...
	float PhysGetSyncCreateTime()
	{
		...
		if (nextTime < simTime)
		{
			...
			return g_ClientGlobalVariables.curtime + nextTime - simTime;
		}
...
//  usercmd input.
	...
	C_CollisionEvent m_Collisions;
	C_StaticCollisionPolyhedronCache m_StaticCollisionPolyhedronCache;
...
bool CClientEntityList<T>::SaveInitEntities(CSaveRestoreData* pSaveData)
{
	...
	entitytable_t* pEntityTable = (entitytable_t*)engineClient->SaveAllocMemory((sizeof(entitytable_t) * number_of_entities), sizeof(char));
	if (!pEntityTable)
		return false;
...
void CClientEntityList<T>::ReadRestoreHeaders(IRestore* pRestore)
{
	...
	entitytable_t* pEntityTable = (entitytable_t*)engineClient->SaveAllocMemory((sizeof(entitytable_t) * nEntities), sizeof(char));
	if (!pEntityTable)
	{
		return;
	}
...
inline IClientEntity* CClientEntityList<T>::CreateEntityByName(const char* className, int iForceEdictIndex, int iSerialNum) {
	...
	if (iForceEdictIndex == 0) {
	...
	else if (iForceEdictIndex>=1 && iForceEdictIndex <= g_ClientGlobalVariables.maxClients) {
		...
		case ENGINEOBJECT_WORLD:
			Error("ENGINEOBJECT_WORLD handled by engineClient!\n");
			break;
		case ENGINEOBJECT_PLAYER:
			Error("ENGINEOBJECT_PLAYER handled by engineClient!\n");
			break;
...
//#endif
	...
	m_PhysWorldObject = PhysCreateWorld_Shared(GetBaseEntity(0), modelinfoclient->GetVCollide(1), g_PhysDefaultObjectParams);
...
void CClientEntityList<T>::LevelShutdown(void)
{
	...
	g_RopeManager->ClearRenderCache();
	if (!m_pWorld) {
		Error("m_pWorld not inited!\n");
	}
...
IClientNetworkable* CClientEntityList<T>::GetClientNetworkable(int entnum)
{
	...
	T* pEnt = BaseClass::LookupEntityByNetworkIndex(entnum);
...
void CClientEntityList<T>::RecomputeHighestEntityUsed(void)
{
	...
	{
		if (BaseClass::LookupEntityByNetworkIndex(i) != NULL)
		{
...
//-----------------------------------------------------------------------------
template<class T>
IClientEntity* CClientEntityList<T>::GetBaseEntity(int entnum) const
{
	T* pEnt = BaseClass::LookupEntityByNetworkIndex(entnum);
...
void CClientEntityList<T>::ToolRecordEntities()
{
	...
	if (!clienttools || !clienttools->IsInRecordingMode())//!ToolsEnabled() || 
		return;
...
void CClientEntityList<T>::SetupBonesOnBaseAnimating(IEngineObjectClient*& pBaseAnimating)
{
	if (!pBaseAnimating->GetMoveParent())
		pBaseAnimating->SetupBones(NULL, -1, -1, g_ClientGlobalVariables.curtime);
}
...
template<class T>
void CClientEntityList<T>::DisplayBoneSetupEnts()
{
...
{
	if (g_ClientGlobalVariables.maxClients != 1)
	{
		return false;
	}
...
void CClientEntityList<T>::CheckInterpolatedVarParanoidMeasurement(IInterpolationContext* pContext)
{
	...
	for (int i = 0; i <= iHighest; i++)
	{
		...
		pEnt->Interpolate(pContext, g_ClientGlobalVariables.curtime);
...
void CClientEntityList<T>::ProcessInterpolatedList(IInterpolationContext* pContext)
{
	...
	for (int iCur = m_InterpolationList.Head(); iCur != m_InterpolationList.InvalidIndex(); iCur = iNext)
	{
		...
		pCur->m_bReadyToDraw = pCur->GetOuter()->Interpolate(pContext, g_ClientGlobalVariables.curtime);
	}
}
...
{
	AddImpactSound(m_impactSounds, pGameData, SOUND_FROM_WORLD, CHAN_STATIC, pObject, surfaceProps, surfacePropsHit, volume, speed);
}
