...
void CBoneMergeCache::Init( IEngineObjectClient *pOwner )
{
	m_pOwner = pOwner;
	m_pFollow = NULL;
	m_pFollowHdr = NULL;
...
void CBoneMergeCache::UpdateCache()
{
	...
	IEngineObjectClient *pTestFollow = m_pOwner->FindFollowedEntity();
...
void CBoneMergeCache::MergeMatchingBones( int boneMask )
{
	...
	bool bWorked = m_pFollow->SetupBones( NULL, -1, m_nFollowBoneSetupMask, g_ClientGlobalVariables.curtime );
...
bool CBoneMergeCache::GetAimEntOrigin( Vector *pAbsOrigin, QAngle *pAbsAngles )
{
	...
	// Get mFollowBone.
	m_pFollow->SetupBones( NULL, -1, m_nFollowBoneSetupMask, g_ClientGlobalVariables.curtime );
