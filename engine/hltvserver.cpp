...
void CHLTVServer::InitClientRecvTables()
{
	...
	for ( pCur = serverGameDLL->GetAllServerClasses(); pCur; pCur=pCur->m_pNext )
	{
		...
		ClientClass * clientclass = new ClientClass("", pCur->m_pNetworkName, NULL, recvt);
...
void CHLTVServer::StartMaster(CGameClient *client)
{
	...
	CPlayerState *player = serverGameClients->GetPlayerState( m_MasterClient->m_nEntityIndex );
	player->hltv = true;
