...
void CCSGameStats::Event_PlayerKilledOther( CBasePlayer *pAttacker, IServerEntity *pVictim, const ITakeDamageInfo&info )
{
	...
	if ( pInflictor )
	{
		...
		else
		{
			...
			if ( V_strcmp( STRING(pInflictor->GetEngineObject()->GetClassname()), "hegrenade_projectile") == 0)
...
void CCSGameStats::FireGameEvent( IGameEvent *event )
{
	...
	else if ( V_strcmp(pEventName, "break_prop") == 0 )
	{
		...
		int entindex = event->GetInt("entindex", 0);
 		CBreakableProp* pProp = static_cast<CBreakableProp*>(EntityList()->GetBaseEntity(entindex));
