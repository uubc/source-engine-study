...
void CCSBot::ComputePartPositions( CCSPlayer *player )
{
	...
	if (studioHdr)
	{
		...
		if (set && maxBoxIndex < set->numhitboxes)
		{
			...
			player->GetEngineObject()->GetHitboxBonePosition( box->bone, info->m_gutPos, angles );

			// head
			box = set->pHitbox( headBox );
			player->GetEngineObject()->GetHitboxBonePosition( box->bone, info->m_headPos, angles );
			...
			player->GetEngineObject()->GetHitboxBonePosition( box->bone, info->m_leftSidePos, angles );

			// right side
			box = set->pHitbox( rightElbowBox );
			player->GetEngineObject()->GetHitboxBonePosition( box->bone, info->m_rightSidePos, angles );
