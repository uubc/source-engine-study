...
//------------------------------------------------------------------------------
void CC_AI_Disable(int nClientIndex)
{
...
//------------------------------------------------------------------------------
void CC_AI_ShowHints(int nClientIndex)
{
...
//------------------------------------------------------------------------------
void CC_AI_ShowHull( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
//------------------------------------------------------------------------------
void CC_AI_ShowConnect( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
// Purpose: Show node connections with lines
//------------------------------------------------------------------------------
void CC_AI_ShowJumpConnect( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
// Purpose: Show node connections with lines
//------------------------------------------------------------------------------
void CC_AI_ShowFlyConnect( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
//------------------------------------------------------------------------------
void CC_AI_ShowGrid( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
//------------------------------------------------------------------------------
void CC_AI_Step(int nClientIndex)
{
	DevMsg("AI Stepping...\n");
...
//------------------------------------------------------------------------------
void CC_AI_Resume(int nClientIndex)
{
	DevMsg("AI Resume...\n");
...
//------------------------------------------------------------------------------
void CC_AI_NextHull( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
...
//------------------------------------------------------------------------------
void CC_AI_Hull( const CCommand &args, int nClientIndex)
{
	if ( !g_pAINetworkManager )
		return;
	...
	{		
		...
		pEnt = EntityList()->FindPickerEntity(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)));
		if ( !pEnt )
		{
...
//------------------------------------------------------------------------------
void CC_AI_Nodes( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//	static char entName[256];	
...
//------------------------------------------------------------------------------
void CC_AI_ShowVisibility( const CCommand &args, int nClientIndex)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	// static char entName[256];	
	...
	CAI_Node* pAINode = FindPickerAINode(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)), NODE_ANY);
	if (pAINode != NULL)
	{
...
//			 netowrk graph
//------------------------------------------------------------------------------
void CC_AI_GraphConnect( const CCommand &args, int nClientIndex)
{
	if ( !g_pAINetworkManager )
		return;
...
//------------------------------------------------------------------------------
void CC_NPC_Bipass( const CCommand &args, int nClientIndex)
{
	SetDebugBits(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)),args[1],OVERLAY_NPC_TRIANGULATE_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Destroy( const CCommand &args, int nClientIndex)
{
	SetDebugBits(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)),args[1],OVERLAY_NPC_ZAP_BIT);
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CC_NPC_Kill( const CCommand &args, int nClientIndex)
{
	SetDebugBits(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)),args[1],OVERLAY_NPC_KILL_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Enemies( const CCommand &args, int nClientIndex)
{
	SetDebugBits(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)),args[1],OVERLAY_NPC_ENEMIES_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Focus( const CCommand &args, int nClientIndex)
{
	SetDebugBits(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)),args[1],OVERLAY_NPC_FOCUS_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Create( const CCommand &args, int nClientIndex)
{
...
// Purpose: Create an NPC of the given type
//------------------------------------------------------------------------------
void CC_NPC_Create_Aimed( const CCommand &args, int nClientIndex)
{
	MDLCACHE_CRITICAL_SECTION();
...
//------------------------------------------------------------------------------
void CC_NPC_DestroyUnselected(int nClientIndex)
{
...
//------------------------------------------------------------------------------
void CC_NPC_Freeze( const CCommand &args, int nClientIndex)
{
	if (FStrEq(args[1], "")) 
	{
		//	
		...
		if (!bFound)
		{
			...
			//
			IServerEntity *pEntity = EntityList()->FindPickerEntity(ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1)));
			if ( pEntity )
			{
...
void CC_NPC_Teleport(int nClientIndex)
{
	CBasePlayer* pPlayer = ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex + 1));
	trace_t tr;
	Vector forward;
...
void CC_NPC_Go(int nClientIndex)
{
	CBasePlayer* pPlayer = ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1));
	trace_t tr;
	Vector forward;
...
//------------------------------------------------------------------------------
void CC_NPC_GoRandom(int nClientIndex)
{
...
//------------------------------------------------------------------------------
void CC_NPC_Reset(int nClientIndex)
{
...
//------------------------------------------------------------------------------
void CC_NPC_Nearest( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_NEAREST_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Route( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_ROUTE_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Select( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_SELECTED_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Combat( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_SQUAD_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Tasks( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_TASK_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Task_Text( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)), args[1], OVERLAY_TASK_TEXT_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Conditions( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)), args[1], OVERLAY_NPC_CONDITIONS_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Viewcone( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1],OVERLAY_NPC_VIEWCONE_BIT);
}
...
//------------------------------------------------------------------------------
void CC_NPC_Relationships( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)),args[1], OVERLAY_NPC_RELATION_BIT );
}
...
//------------------------------------------------------------------------------
void CC_NPC_ViewSteeringRegulations( const CCommand &args, int nClientIndex)
{
	SetDebugBits( ToBasePlayer(EntityList()->GetPlayerByIndex(nClientIndex+1)), args[1], OVERLAY_NPC_STEERING_REGULATIONS);
}
...
void CC_NPC_ViewSteeringRegulationsAll(int nClientIndex)
{
	CAI_BaseNPC *pNPC = NextEntByClass( (CAI_BaseNPC *)NULL );
