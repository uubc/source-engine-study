...
int CNPC_Citizen::SelectSchedule()
{
	...
	if (GetEngineObject()->GetMoveType() == MOVETYPE_NONE )
	{
		...
		Assert(GetEngineObject()->GetMoveParent() && FClassnameIs(GetEngineObject()->GetMoveParent()->GetOuter(), "func_tracktrain"));
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Citizen::SimpleUse( IServerEntity *pActivator, IServerEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
		if (GetEngineObject()->HasSpawnFlags(SF_CITIZEN_NOT_COMMANDABLE) || IRelationType( (CBaseEntity*)pActivator ) == D_NU )
		{
...
void CNPC_Citizen::CommanderUse( IServerEntity *pActivator, IServerEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pActivator == EntityList()->GetLocalPlayer() )
	{
...
void CNPC_Citizen::TogglePlayerSquadState()
{
	...
	if ( !IsInPlayerSquad() )
	{
		...
		else if ( m_FollowBehavior.GetFollowTarget() == EntityList()->GetLocalPlayer() )
		{
			SpeakCommandResponse( TLK_STARTFOLLOW );
		}
	}
...
{
	TossHealthKit(ToBasePlayer(EntityList()->GetLocalPlayer()), Vector(48.0f, 0.0f, 0.0f)  );
}
...
bool CNPC_Citizen::ShouldLookForHealthItem()
{
	...
	if( AI_IsSinglePlayer() && EntityList()->GetLocalPlayer()->GetHealth() <= EntityList()->GetLocalPlayer()->GetHealth() * 0.75f )
		return false;
...
void CNPC_Citizen::DeathSound( const CTakeDamageInfo &info )
{
	...
	const char* soundname = "NPC_Citizen.Die";
	CPASAttenuationFilter filter(this, soundname);
