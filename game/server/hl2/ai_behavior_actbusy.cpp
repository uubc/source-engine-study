			...
			if ( HasAnimForActBusy( iBusyAnim, BA_BUSY ) )
			{
				...
				if ( GetHintNode() && GetHintNode()->Lock( GetOuter() ) )
				{
					...
					if( m_hActBusyGoal && m_hActBusyGoal->m_iszSeeEntityName != NULL_STRING )
					{
						...
						m_hSeeEntity.Set((CBaseEntity*)EntityList()->FindEntityByName(NULL, m_hActBusyGoal->m_iszSeeEntityName) );
					}
...
void CAI_ActBusyBehavior::CollectSafeZoneVolumes( CAI_ActBusyGoal *pActBusyGoal )
{
	...
	{
		IServerEntity *pVolume = EntityList()->FindEntityByName( NULL, pActBusyGoal->m_iszSafeZoneVolume );

		while( pVolume != NULL )
		{
			busysafezone_t newSafeZone;
			pVolume->GetEngineObject()->WorldSpaceAABB( &newSafeZone.vecMins, &newSafeZone.vecMaxs );
			m_SafeZones.AddToTail( newSafeZone );
			pVolume = EntityList()->FindEntityByName( pVolume, pActBusyGoal->m_iszSafeZoneVolume );
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CAI_ActBusyGoal::GetBusyBehaviorForNPC( const char *pszActorName, IServerEntity *pActivator, IServerEntity *pCaller, const char *sInputName )
{
	IServerEntity *pEntity = EntityList()->FindEntityByName( NULL, MAKE_STRING(pszActorName), NULL, pActivator, pCaller );
	if ( !pEntity )
	{
	...
	return GetBusyBehaviorForNPC((CBaseEntity*)pEntity, sInputName );
}
...
void CAI_ActBusyGoal::InputForceNPCToActBusy( inputdata_t &inputdata )
{
	...
	{	
		...
		IServerEntity *pEntity = EntityList()->FindEntityByName( NULL, pszParam, NULL, inputdata.pActivator, inputdata.pCaller );
		if ( pEntity )
		{
	...
	while ( pszParam )
	{
		...
		{
			pSeeEntity = EntityList()->FindEntityByName( NULL, pszParam+4 );
		}
		else if ( pszParam[0] == '$' )
		{
	...
	pBehavior->SetBusySearchRange( m_flBusySearchRange );
	pBehavior->ForceActBusy( this, pHintNode, flMaxTime, m_bVisibleOnly, bTeleport, bUseNearestBusy, (CBaseEntity*)pSeeEntity, activity );
}
...
void CAI_ActBusyQueueGoal::InputActivate( inputdata_t &inputdata )
{
	if ( !IsActive() )
	{
		...
		for ( int i = 0; i < MAX_QUEUE_NODES; i++ )
		{
			...
			IServerEntity *pEntity = EntityList()->FindEntityByName( NULL, m_iszNodes[i] );
			if ( !pEntity )
			{
		...
		// Find the exit node
		m_hExitNode = (CBaseEntity*)EntityList()->FindEntityByName( NULL, m_iszExitNode );
		if ( !m_hExitNode )
		{
