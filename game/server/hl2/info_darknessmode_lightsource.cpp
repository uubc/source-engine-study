	...
	for ( int i = m_LightSources.Count() - 1; i >= 0; i-- )
	{
		...
		if ( pLightSource )
		{
			if ( bDebug )
			{
				...
				pLightSource->GetEngineObject()->SetNextThink( gpGlobals->curtime );
			}
			else
			{
...
void AddEntityToDarknessCheck( CBaseEntity *pEntity, float flLightRadius /*=DARKNESS_LIGHTSOURCE_SIZE*/ )
{
	...
	CInfoDarknessLightSource *pLightSource = (CInfoDarknessLightSource *)EntityList()->CreateEntityByName( "info_darknessmode_lightsource" );
	if ( pLightSource )	
	{
...
void RemoveEntityFromDarknessCheck( CBaseEntity *pEntity )
{
	...
	while ( pChild )
	{
		...
		{
			EntityList()->DestroyEntity( pPrevChild->GetOuter() );
		}
	}
}
