...
bool CPropJeep::CheckWater( void )
{
	...
	for ( int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel )
	{
		...
		{
			m_WaterData.m_bWheelInWater[iWheel] = ( UTIL_PointContents(EntityList(), m_WaterData.m_vecWheelContactPoints[iWheel] ) & MASK_WATER ) ? true : false;
	...
	m_WaterData.m_bBodyInWater = ( UTIL_PointContents(EntityList(), vecEnginePoint ) & MASK_WATER ) ? true : false;
...
void CPropJeep::FireChargedCannon( void )
{
	...
	trace_t	tr;
	UTIL_TraceLine(EntityList(), m_vecGunOrigin, endPos, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
	...
	if ( tr.DidHitWorld() && !(tr.surface.flags & SURF_SKY) )
	{
		...
		UTIL_TraceLine(EntityList(), testPos, tr.endpos, MASK_SHOT, GetDriver(), COLLISION_GROUP_NONE, &tr );
...
//-----------------------------------------------------------------------------
void CPropJeep::Use( IServerEntity *pActivator, IServerEntity *pCaller, USE_TYPE useType, float value )
{
...
void CPropJeep::SpawnPerchedSeagull( void )
{
	...
	Vector vecOrigin;
	QAngle vecAngles;
	int iAttachment = GetEngineObject()->GetModelPtr()->Studio_FindRandomAttachment( "seagull_perch" );
	if ( iAttachment == -1 )
		return;
	...
	pSeagull->GetEngineObject()->SetOwnerEntity( this->GetEngineObject() );
...
void CPropJeep::AddSeagullPoop( const Vector &vecOrigin )
{
	...
	for ( int i = 0; i < iDecals; i++ )
	{
		...
		trace_t tr;
		UTIL_TraceLine(EntityList(), vecPoop, vecPoop - Vector(0,0,512), MASK_SHOT, m_hSeagull, COLLISION_GROUP_NONE, &tr );
