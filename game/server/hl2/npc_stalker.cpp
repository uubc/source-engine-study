...
bool CNPC_Stalker::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if( enemyClass == CLASS_BULLSEYE && pEnemy->GetEngineObject()->GetMoveParent() )
	{
		// This bullseye is in heirarchy with something. If that
		// something is held by the physcannon, this bullseye is 
		// NOT a valid enemy.
		IPhysicsObject *pPhys = pEnemy->GetEngineObject()->GetMoveParent()->GetOuter()->GetEngineObject()->VPhysicsGetObject();
...
void CNPC_Stalker::KillAttackBeam(void)
{
	...
	EntityList()->DestroyEntity( m_pLightGlow );
