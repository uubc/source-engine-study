...
void CWeaponShotgun::PrimaryAttack( void )
{
	...
	CSoundEnt::InsertSound( SOUND_COMBAT, GetEngineObject()->GetAbsOrigin(), SOUNDENT_VOLUME_SHOTGUN, 0.2, GetOwner() );
...
void CWeaponShotgun::SecondaryAttack( void )
{
	...
	CSoundEnt::InsertSound( SOUND_COMBAT, GetEngineObject()->GetAbsOrigin(), SOUNDENT_VOLUME_SHOTGUN, 0.2 );
...
void CWeaponShotgun::ItemPostFrame( void )
{
	...
	if ((m_bDelayedFire2 || pOwner->m_nButtons & IN_ATTACK2)&&(m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		// Fire underwater?
		else if (GetOwner()->GetEngineObject()->GetWaterLevel() == 3 && m_bFiresUnderwater == false)
		{
			WeaponSound(EMPTY);
	...
	else if ( (m_bDelayedFire1 || pOwner->m_nButtons & IN_ATTACK) && m_flNextPrimaryAttack <= gpGlobals->curtime)
	{
		...
		// Fire underwater?
		else if (pOwner->GetEngineObject()->GetWaterLevel() == 3 && m_bFiresUnderwater == false)
		{
			WeaponSound(EMPTY);
			m_flNextPrimaryAttack = gpGlobals->curtime + 0.2;
			return;
