...
int CNPC_BaseZombie::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	if( tr.m_pEnt == GetEnemy() || 
		tr.m_pEnt->IsNPC() ||
		(tr.m_pEnt->GetTakeDamage() == DAMAGE_YES && (dynamic_cast<CBreakableProp*>(tr.m_pEnt))))
	{
...
void CNPC_BaseZombie::DieChopped( const CTakeDamageInfo &info )
{
	...
	if ( pTorsoGib )
	{
		...
		pTorsoGib->GetEngineObject()->SetOwnerEntity( this->GetEngineObject());
...
void CNPC_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_POPHEADCRAB )
	{
		...
		int boneIndex = GetInteractionPartner()->GetEngineObject()->LookupBone( token );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseZombie::StartTouch( IServerEntity *pOther )
{
