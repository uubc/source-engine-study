...
void CPlayerPickupController::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	...
	CHL2_Player *pOwner = ToHL2Player( pPlayer );
	if ( pOwner )
	{
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if ( m_pPlayer )
	{
		CHL2_Player *pOwner = ToHL2Player( m_pPlayer );
		if ( pOwner )
		{
...
bool CWeaponPhysCannon::DropIfEntityHeld( CBaseEntity *pTarget )
{
	...
	IServerEntity *pHeld = GetGrabController()->GetAttached();
...
void CWeaponPhysCannon::PrimaryAttack( void )
{
	...
	if( m_bActive )
	{
		...
		if ( pHeld != NULL )
		{
			float heldDist = pHeld->GetEngineObject()->CalcDistanceFromPoint(pOwner->WorldSpaceCenter() );
...
CBaseEntity * CHL2_Player::GetPlayerHeldEntity()
{
	...
	{
		pObject = (CBaseEntity*)pPlayerPickupController->GetGrabController()->GetAttached();
	}
