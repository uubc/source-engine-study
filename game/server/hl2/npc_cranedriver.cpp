...
void CNPC_CraneDriver::InputForcePickup( inputdata_t &inputdata )
{
	...
	if ( iszPickupName != NULL_STRING )
	{
		...
		m_hCrane->TurnMagnetOff();
		m_hPickupTarget = (CBaseEntity*)EntityList()->FindEntityByName( NULL, iszPickupName, NULL, inputdata.pActivator, inputdata.pCaller );
...
void CNPC_CraneDriver::InputForceDrop( inputdata_t &inputdata )
{
	...
	{
		IServerEntity *pEntity = EntityList()->FindEntityByName( NULL, iszDropName, NULL, inputdata.pActivator, inputdata.pCaller );
		if ( !pEntity )
		{
