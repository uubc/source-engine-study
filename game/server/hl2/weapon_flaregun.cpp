...
CFlare *CFlare::Create( Vector vecOrigin, QAngle vecAngles, CBaseEntity *pOwner, float lifetime )
{
	...
	pFlare->GetEngineObject()->SetOwnerEntity(pOwner ? pOwner->GetEngineObject() : NULL);
...
void CFlare::FlareTouch( IServerEntity *pOther )
{
	...
	if ( ( m_nBounces < 10 ) && (GetEngineObject()->GetWaterLevel() < 1 ) )
	{
	...
	else
	{
		...
		GetEngineObject()->SetOwnerEntity( this->GetEngineObject() );
