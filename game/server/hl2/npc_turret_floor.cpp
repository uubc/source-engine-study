...
void CNPC_FloorTurret::Retire( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_CLOSE )
	{
		...
		if ( UpdateFacing() == false )
		{
			...
			const char* soundname = "NPC_FloorTurret.Retire";
...
void CNPC_FloorTurret::Deploy( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_OPEN )
	{
		...
		const char* soundname = "NPC_FloorTurret.Deploy";
		CPASAttenuationFilter filter(this, soundname);
...
bool CNPC_FloorTurret::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( UTIL_PointContents(EntityList(), EyePosition()) & MASK_SHOT )
		return false;
	...
	if ( IsCitizenTurret() )
	{
		...
		if (GetEngineObject()->HasSpawnFlags( SF_FLOOR_TURRET_OUT_OF_AMMO ) == false || m_bBlinkState )
		{
			...
			if ( m_bBlinkState == false )
			{
				...
				const char* soundname = "NPC_FloorTurret.AlarmPing";
				CPASAttenuationFilter filter(this, soundname);
	...
	if ( !IsBeingCarriedByPlayer() )
	{
		...
		else
		{
			if (GetEngineObject()->HasSpawnFlags( SF_FLOOR_TURRET_OUT_OF_AMMO ) == false )
			{
				...
				if ( !m_bNoAlarmSounds )
				{
					const char* soundname = "NPC_FloorTurret.Alarm";
					CPASAttenuationFilter filter(this, soundname);
...
void CNPC_FloorTurret::BreakThink( void )
{
	...
	{
		const char* soundname = "NPC_FloorTurret.Destruct";
		CPASAttenuationFilter filter(this, soundname);
...
void CNPC_FloorTurret::InputSelfDestruct( inputdata_t &inputdata )
{
	...
	if ( m_hFizzleEffect != NULL )
	{
		...
		EntityList()->DispatchSpawn( m_hFizzleEffect );
