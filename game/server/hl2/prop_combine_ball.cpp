...
CBasePlayer *CPropCombineBall::HasPhysicsAttacker( float dt )
{
	...
	// Must be a player
	if (GetEngineObject()->GetOwnerEntity()->IsPlayer() == false )
		return NULL;
...
void CPropCombineBall::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	{
		m_pGlowTrail->TurnOff();
		m_pGlowTrail->GetEngineObject()->SetRenderColor( 0, 0, 0, 0 );
	}
...
void CPropCombineBall::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	{
		m_pGlowTrail->TurnOn();
		m_pGlowTrail->GetEngineObject()->SetRenderColor( 255, 255, 255, 255 );
	}
...
bool CPropCombineBall::IsAttractiveTarget( CBaseEntity *pEntity )
{
	...
	else
	{
		...
		if (GetEngineObject()->GetOwnerEntity() )
		{
			...
			if(GetEngineObject()->GetOwnerEntity()->IsPlayer() )
			{
...
void CFuncCombineBallSpawner::GrabBallTouch( IServerEntity *pOther )
{
	...
	pBall->GetEngineObject()->SetOwnerEntity( NULL );
	pBall->SetSpawner( this );
