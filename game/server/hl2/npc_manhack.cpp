...
bool CNPC_Manhack::CorpseGib( const ITakeDamageInfo&info )
{
	...
	PropBreakableCreateAll(GetEngineObject()->GetModelIndex(), NULL, GetEngineObject()->GetAbsOrigin(), GetEngineObject()->GetAbsAngles(), vecGibVelocity, vecGibAVelocity, 1.0, 60, COLLISION_GROUP_DEBRIS );
...
void CNPC_Manhack::MoveExecute_Alive(float flInterval)
{
	...
	if( m_flWaterSuspendTime > gpGlobals->curtime )
	{ 
		if( UTIL_PointContents(EntityList(), GetEngineObject()->GetAbsOrigin() ) & (CONTENTS_WATER|CONTENTS_SLIME) )
		{
...
void CNPC_Manhack::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	{
		m_pPrevOwner.Set(GetEngineObject()->GetOwnerEntity() ? (CBaseEntity*)GetEngineObject()->GetOwnerEntity()->GetServerEntity() : NULL);
...
{
	GetEngineObject()->SetOwnerEntity(m_pPrevOwner.Get() ? m_pPrevOwner.Get()->GetEngineObject() : NULL);
