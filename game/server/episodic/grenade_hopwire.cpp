...
bool CGravityVortexController::KillNPCInRange( CBaseEntity *pVictim, IPhysicsObject **pPhysObj )
{
	...
	if ( pBCC != NULL && pBCC->CanBecomeRagdoll() )
	{
		...
		CBaseEntity *pRagdoll = pBCC->CreateServerRagdoll( 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true );
		pBCC->FixupBurningServerRagdoll(pRagdoll);
...
void CGrenadeHopwire::Detonate( void )
{
	...
	trace_t	tr;
	UTIL_TraceLine(EntityList(), GetEngineObject()->GetAbsOrigin(), GetEngineObject()->GetAbsOrigin() + Vector( 0, 0, MAX_HOP_HEIGHT*2 ), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr );
