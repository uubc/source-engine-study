...
{
	CWindowPane *pGlass = (CWindowPane*)EntityList()->CreateEntityByName( "window_pane" );
	if ( !pGlass )
	{
...
void CBreakableSurface::Spawn(void)
{
	...
	IMaterial* pMaterial;
	modelinfo->GetModelMaterials( const_cast<model_t*>(GetEngineObject()->GetModel()), 1, &pMaterial );
