...
public:
	...
	virtual void				CSEjectBrass( const Vector &vecPosition, const QAngle &angVelocity, int nType, int nShellType, IClientPlayer*pShooter ) = 0;
	...
	virtual C_LocalTempEntity	*ClientProjectile( const Vector& vecOrigin, const Vector& vecVelocity, const Vector& vecAccel, int modelindex, int lifetime, CBaseEntity *pOwner) = 0;//, const char *pszImpactEffect = NULL, const char *pszParticleEffect = NULL 
};
...
public:
	...
	void					CSEjectBrass( const Vector &vecPosition, const QAngle &angAngles, int nType, int nShellType, IClientPlayer*pShooter );
