...
void CHudMenu::SelectMenuItem( int menu_item )
{
	...
	if ( (menu_item > 0) && (m_bitsValidSlots & (1 << (menu_item-1))) )
	{
		...
		// Pulse the selection
		g_pGameRules->GetViewportAnimationController()->StartAnimationSequence("MenuPulse");

		// remove the menu quickly
		m_bMenuTakesInput = false;
		m_flShutoffTime = gpGlobals->realtime + m_flOpenCloseTime;
		g_pGameRules->GetViewportAnimationController()->StartAnimationSequence("MenuClose");
	}
}
...
void CHudMenu::ShowMenu( const char * menuName, int validSlots )
{
	...
	g_pGameRules->GetViewportAnimationController()->StartAnimationSequence("MenuOpen");
	m_nSelectedItem = -1;
