...
inline void CClient_Precipitation::RenderParticle( CPrecipitationParticle* pParticle, CMeshBuilder &mb )
{
	...
	Vector3DMultiplyPosition(g_pViewRender->CurrentWorldToViewMatrix(), pParticle->m_Pos, start );
	if ( start.z > -1 )
		return;

	Vector3DMultiply(g_pViewRender->CurrentWorldToViewMatrix(), pParticle->m_Velocity, delta );
	...
	if ( m_nPrecipType == PRECIPITATION_TYPE_SNOW )
	{
		...
		if ( pParticle->m_Mass > 1.0f )
		{
			...
			Vector3DMultiply(g_pViewRender->CurrentWorldToViewMatrix(), spiral, camSpiral );
...
void CClient_Precipitation::CreateWaterSplashes()
{
	for ( int i=0; i < m_Splashes.Count(); i++ )
	{
		...
		if (g_pViewRender->CurrentViewForward().Dot( vSplash - g_pViewRender->CurrentViewOrigin() ) > 1 )
		{
...
void CClient_Precipitation::Render()
{
	...
	if (g_pViewRender->CurrentViewID() == VIEW_MONITOR )
		return;

	if (g_pViewRender->GetDrawFlags() & (DF_RENDER_REFLECTION | DF_RENDER_REFRACTION) )
		return;
...
public:
	...
	virtual void LevelInitPostEntity()
	{
		if ( r_RainHack.GetInt() )
		{
			CClient_Precipitation *pPrecipHackEnt = (CClient_Precipitation*)EntityList()->CreateEntityByName( "CClient_Precipitation" );
	...
	virtual void LevelShutdownPreEntity()
	{
		...
		{
			EntityList()->DestroyEntity(g_pPrecipHackEnt);// ->Release();
		}
		m_bLevelInitted = false;
	}
...
bool SnowFallManagerCreate( CClient_Precipitation *pSnowEntity )
{
	if ( !s_pSnowFallMgr )
		{
		s_pSnowFallMgr = (CSnowFallManager*)EntityList()->CreateEntityByName( "CSnowFallManager");
...
void SnowFallManagerDestroy( void )
{
	if ( s_pSnowFallMgr )
	{
		EntityList()->DestroyEntity(s_pSnowFallMgr);
		s_pSnowFallMgr = NULL;
	}
}
