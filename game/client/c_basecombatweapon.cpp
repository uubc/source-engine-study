...
static inline bool ShouldDrawLocalPlayerViewModel( void )
{
	...
	return false;
#else
	return !EntityList()->GetLocalPlayer()->AsHandlePlayer()->ShouldDrawLocalPlayer();
#endif
}
...
void C_BaseCombatWeapon::OnRestore()
{
	...
	if (GetOwner() == (C_BasePlayer*)EntityList()->GetLocalPlayer())
	{
...
ShadowType_t C_BaseCombatWeapon::ShadowCastType()
{
	...
	if (IsCarriedByLocalPlayer() && !EntityList()->GetLocalPlayer()->AsHandlePlayer()->ShouldDrawLocalPlayer())
		return SHADOWS_NONE;
...
/*
	...
	{
		C_BaseEntity *pEnt = EntityList()->GetBaseEntityFromHandle( tr.index );
		if ( pEnt )
		{
...
bool C_BaseCombatWeapon::IsCarriedByLocalPlayer( void )
{
	...
	return ( GetOwner() == (C_BasePlayer*)EntityList()->GetLocalPlayer() );
}
...
{
	return IsCarriedByLocalPlayer() && !EntityList()->GetLocalPlayer()->AsHandlePlayer()->ShouldDrawLocalPlayer();
}
...
bool C_BaseCombatWeapon::GetShootPosition( Vector &vOrigin, QAngle &vAngles )
{
	...
	if ( pEnt )
	{
		if ( pEnt == (C_BasePlayer*)EntityList()->GetLocalPlayer() )
		{
