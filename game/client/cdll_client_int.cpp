...
void CHLClient::HudUpdate( bool bActive )
{
	...
	hudlcd->SetGlobalStat( "(time_int)", UTIL_VarArgs( "%d", (int)gpGlobals->curtime ) );
	hudlcd->SetGlobalStat( "(time_float)", UTIL_VarArgs( "%.2f", gpGlobals->curtime ) );
...
void CHLClient::InvalidateMdlCache()
{
	...
	{
		pAnimating = dynamic_cast<C_BaseEntity*>(pEntity);
...
void SimulateEntities()
{
	...
	EntityList()->PerformThinkFunctions();
