...
//-----------------------------------------------------------------------------
void C_NPC_Puppet::BeforeBuildTransformations( IStudioHdr *pStudioHdr, Vector *pos, Quaternion q[], const matrix3x4_t& cameraTransform, int boneMask, CBoneBitList &boneComputed )
{
...
void C_NPC_Puppet::AccumulateLayers( IBoneSetup &boneSetup, Vector pos[], Quaternion q[], float currentTime )
{
	...
	for (j = 0; j < C_AnimationLayer::MAX_OVERLAYS; j++)
	{
		...
		if (i < pTarget->m_AnimOverlay.Count())
		{
			...
			if (fWeight > 0)
			{
				const char *pSequenceName = pTarget->GetEngineObject()->GetSequenceName( pTarget->m_AnimOverlay[i].m_nSequence );
