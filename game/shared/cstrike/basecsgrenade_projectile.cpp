...
END_NETWORK_TABLE()
	...
	int CBaseCSGrenadeProjectile::DrawModel( int flags )
	{
		...
		if ( GetThrower() != (C_BasePlayer*)EntityList()->GetLocalPlayer() )
		{
	...
	void CBaseCSGrenadeProjectile::Splash()
	{
		...
		UTIL_TraceLine(EntityList(), centerPoint, centerPoint + vecTrace, MASK_WATER, NULL, COLLISION_GROUP_NONE, &tr );
