	...
	CBasePlayer *owner, const Vector& org, const Vector& mins, const Vector& maxs, bool& validspot )
{
	...
	spot->GetEngineObject()->SetOwnerEntity(owner ? owner->GetEngineObject() : NULL);
	spot->Spawn();
...
void CHL2GameMovement::SetGroundEntity( trace_t *pm )
{
	...
	if ( newGround && newGround->GetEngineObject()->GetCollisionGroup() == HL2COLLISION_GROUP_COMBINE_BALL_NPC )
	{
		return;
	}
