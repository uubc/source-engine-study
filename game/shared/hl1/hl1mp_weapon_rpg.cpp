...
void CRpgRocket::RocketTouch( IServerEntity *pOther )
{
	...
	g_pSoundEmitterSystem->StopSound(this, "Weapon_RPG.RocketIgnite" );
	ExplodeTouch( pOther );
}
...
CRpgRocket *CRpgRocket::Create( const Vector &vecOrigin, const QAngle &angAngles, CBasePlayer *pentOwner )
{
	...
	pRocket->Spawn();
	pRocket->GetEngineObject()->SetOwnerEntity(pentOwner ? pentOwner->GetEngineObject() : NULL);
...
int CLaserDot::DrawModel( int flags )
{
	...
	C_HL1MP_Player* pOwner = ToHL1MPPlayer(GetEngineObject()->GetOwnerEntity() ? GetEngineObject()->GetOwnerEntity()->GetOuter() : NULL);
