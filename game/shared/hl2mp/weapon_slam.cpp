...
void CWeapon_SLAM::TripmineAttach( void )
{
	...
	UTIL_TraceLine(EntityList(), vecSrc, vecSrc + (vecAiming * 128), MASK_SOLID, pOwner, COLLISION_GROUP_NONE, &tr );
...
void CWeapon_SLAM::StartTripmineAttach( void )
{
	...
	UTIL_TraceLine(EntityList(), vecSrc, vecSrc + (vecAiming * 128), MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
...
void CWeapon_SLAM::SatchelAttach( void )
{
	...
	if (tr.fraction < 1.0)
	{
		...
		if (pEntity && !(pEntity->GetEngineObject()->GetFlags() & FL_CONVEYOR))
		{
			...
			pSatchel->m_bIsLive			= true;
			pSatchel->SetThrower( GetOwner() );
			pSatchel->GetEngineObject()->SetOwnerEntity( ((CBaseEntity*)GetOwner())->GetEngineObject() );
			pSatchel->m_pMyWeaponSLAM	= this;
...
bool CWeapon_SLAM::CanAttachSLAM( void )
{
	...
	UTIL_TraceLine(EntityList(), vecSrc, vecEnd, MASK_SOLID, pOwner, COLLISION_GROUP_NONE, &tr );
