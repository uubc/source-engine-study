...
CCrossbowBolt::~CCrossbowBolt( void )
{
	if ( m_pGlowSprite )
	{
		EntityList()->DestroyEntity( m_pGlowSprite );
	}
}
...
bool CCrossbowBolt::CreateSprites( void )
{
	...
	m_pGlowSprite = CSprite::SpriteCreate( "sprites/light_glow02_noz.vmt", GetEngineObject()->GetLocalOrigin(), false );
...
//			NDebugOverlay::Box( GetAbsOrigin(), Vector( -16, -16, -16 ), Vector( 16, 16, 16 ), 0, 0, 255, 0, 10 );
			...
			if ( tr2.m_pEnt == NULL || ( tr2.m_pEnt && tr2.m_pEnt->GetEngineObject()->GetMoveType() == MOVETYPE_NONE ) )
			{
				...
				data.m_hEntity = GetWorldEntity();//need check
	...
	else
	{
		...
		if ( pOther->GetEngineObject()->GetMoveType() == MOVETYPE_NONE && !( tr.surface.flags & SURF_SKY ) )
		{
			...
			if ( ( hitDot < 0.5f ) && ( speed > 100 ) )
			{
				...
				GetEngineObject()->SetLocalAngles( reflectAngles );
...
void CWeaponCrossbow::CreateChargerEffects( void )
{
	...
	m_hChargerSprite = CSprite::SpriteCreate( CROSSBOW_GLOW_SPRITE, GetEngineObject()->GetAbsOrigin(), false );
