...
void CWeaponStunStick::SetStunState( bool state )
{
	...
	if ( m_bActive )
	{
		...
		GetEngineObject()->GetAttachment( 1, vecAttachment, vecAttachmentAngles );
...
void C_WeaponStunStick::DrawThirdPersonEffects( void )
{
	...
	if ( InSwing() )
	{
		...
		if ( gpGlobals->frametime != 0.0f && ( random->RandomInt( 0, 5 ) == 0 ) )
		{
			...
			beamInfo.m_pStartEnt= (CBaseEntity*)EntityList()->GetEnt( BEAMENT_ENTITY( entindex() ) );
			beamInfo.m_pEndEnt	= (CBaseEntity*)EntityList()->GetEnt( BEAMENT_ENTITY( entindex() ) );
